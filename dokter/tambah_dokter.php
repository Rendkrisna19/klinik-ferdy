...
<body class="bg-white flex min-h-screen font-sans font-modify">
    ...
    <main class="flex-1 ml-64 p-8">
        <h1 class="text-4xl font-bold mb-8 text-green-600">Tambah Dokter Baru</h1>
        ...
        <?php if ($success): ?>
        <div class="mb-6 p-4 bg-green-100 text-green-700 rounded border border-green-300">
            <?= htmlspecialchars($success) ?></div>
        <?php endif; ?>

        <form method="POST" class="bg-white p-8 rounded-lg shadow-lg max-w-lg border border-green-200">
            <div class="mb-6">
                <label for="nama" class="block mb-2 font-semibold text-gray-800">Nama Dokter</label>
                <input id="nama" name="nama" type="text" required
                    class="w-full px-4 py-3 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500" />
            </div>
            ...
            <div class="mb-8">
                ...
                <textarea id="alamat" name="alamat" rows="4" required
                    class="w-full px-4 py-3 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 resize-none"></textarea>
            </div>

            <button type="submit" name="tambah_dokter"
                class="bg-green-600 hover:bg-green-700 text-white font-semibold px-6 py-3 rounded-md transition-colors duration-200">
                Tambah Dokter
            </button>
        </form>
    </main>
